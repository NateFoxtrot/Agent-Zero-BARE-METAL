...
class Settings(TypedDict):
    ...
    # --- GOOGLE CONFIG ---
    google_auth_mode: Literal['api_key', 'vertex_json', 'hybrid']
...
def convert_out(settings: Settings) -> SettingsOutput:
    ...
    google_fields.append({
        ...
        "description": "Choose how to connect to Google/Gemini models.",
        "type": "select",
    ...
    google_fields.append({
        "id": "google_vertex_project",
        "title": "Vertex Project ID",
        "description": "Google Cloud Project ID (e.g., 'my-agent-project-123'). Required to construct the correct API endpoint.",
        "type": "text",
        "value": settings.get("google_vertex_project", "")
    })

    google_fields.append({
        "id": "google_vertex_location",
        "title": "Vertex Region",
        "description": "Google Cloud Region (e.g., 'us-central1'). Defaults to us-central1.",
        "type": "text",
        "value": settings.get("google_vertex_location", "us-central1")
    })

    google_desc = """<div style="display: flex; align-items: center; gap: 15px;"><img src="https://www.gstatic.com/images/branding/product/1x/google_cloud_48dp.png" style="width: 48px; filter: drop-shadow(0 0 2px rgba(255,255,255,0.5));"><div><h4 style="margin:0; color: #669df6;">GOOGLE CLOUD / VERTEX AI</h4><p style="margin:5px 0;">Authentication & Credential Bridge</p></div></div>"""
    ...
    sentinel_fields.append({
        ...
        "description": "Toggle the background hardware and process supervisor.",
        "type": "switch",
    ...
    sentinel_fields.append({
        ...
        "description": "Select the local Ollama model to power the Watchdog.",
        "type": "select",
    ...
    memory_fields.append(
        {
            ...
            "options": [{"value": subdir, "label": subdir} for subdir in files.get_subdirectories("memory", exclude="embeddings")]
        }
    )
    ...
    secrets_fields.append({
        "id": "secrets",
        "title": "Secrets Store",
        "description": "Store secrets and credentials in .env format e.g. EMAIL_PASSWORD=\"s3cret-p42246230w0rd\", one item per line. You can use comments starting with # to add descriptions for the agent. See <a href=\"javascript:openModal('settings/secrets/example-secrets.html')\">example</a>.<br>These variables are not visile to LLMs and in chat history, they are being masked. ⚠️ only values with length >= 4 are being masked to prevent false positives. ",
        "type": "textarea",
        "value": secrets,
        "style": "height: 20em",
    })
    ...
    result: SettingsOutput = {
        "sections": [
            google_section, # ADDED GOOGLE CONFIG
            sentinel_section, # ADDED SENTINEL
            agent_section,
            chat_model_section,
            util_model_section,
...
def normalize_settings(settings: Settings) -> Settings:
    ...
    # --- GOOGLE AUTH LOGIC ---
    # Automatically export/unset environment variable based on selection
    ...
    if mode == "vertex_json" or mode == "hybrid":
        if json_path and os.path.exists(json_path):
            ...
            # Export Project/Location variables if set
            if copy.get("google_vertex_project"):
                os.environ["VERTEXAI_PROJECT"] = copy["google_vertex_project"]
                dotenv.save_dotenv_value("VERTEXAI_PROJECT", copy["google_vertex_project"])
                
            if copy.get("google_vertex_location"):
                os.environ["VERTEXAI_LOCATION"] = copy["google_vertex_location"]
                dotenv.save_dotenv_value("VERTEXAI_LOCATION", copy["google_vertex_location"])
            
            # --- AUTO-PROVIDER SWITCHING LOGIC ---
            # Automatically switch Google providers to 'vertex_ai' if mode is Vertex
            if mode == "vertex_json":
                if copy["util_model_provider"] == "google":
                    copy["util_model_provider"] = "vertex_ai"
                if copy["embed_model_provider"] == "google":
                    copy["embed_model_provider"] = "vertex_ai"
                if copy["browser_model_provider"] == "google":
...
def get_runtime_config(set: Settings):
    if runtime.is_dockerized():
        ...
        if host.endswith("/"):
            host = host[:-1]
        return {
            "code_exec_ssh_enabled": False, # FORCE FALSE
