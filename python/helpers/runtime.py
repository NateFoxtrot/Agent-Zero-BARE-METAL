...
def initialize():
    ...
    # Bare Metal Fix: Use store_true for boolean flags to ensure correct False default
    parser.add_argument(
        "--dockerized", action="store_true", help="Run in dockerized mode"
    )
    parser.add_argument(
...
def is_development() -> bool:
    # Bare Metal Fix: Decouple dev mode from docker status.
    # Only true if explicitly set via flag.
...
async def call_development_function(
    ...
    if is_development():
        ...
        # Bare Metal / Production: Run locally
...
def call_development_function_sync(
    ...
    def run_in_thread():
        # Setup new loop for this thread if needed to avoid loop conflicts
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        try:
            if inspect.iscoroutinefunction(func):
                result = loop.run_until_complete(call_development_function(func, *args, **kwargs))
            else:
                result = loop.run_until_complete(call_development_function(func, *args, **kwargs))
